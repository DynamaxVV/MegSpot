...
它将两组图片或图片文件夹组织成可追溯的对比任务，重点优化批量配对、三种对比布局、文件变化保护、LabelPlus 标注审校和大图/快速翻页时的资源管理，适合不希望上传素材、需要逐组核对图片的本地工作流。
...
- **更可控的配对规则**：支持文件名匹配以及按组独立的“按顺序匹配”（忽略文件名按序配对）；文件名匹配忽略扩展名和大小写；剩余图片按两侧排序顺序配对；文件夹只读取直属图片并按绝对路径去重。主页添加文件夹时，连续单子文件夹嵌套会定位到最里面一层。
- **三种对比布局**：并排、单页、分割；支持方向键临时查看另一侧和切换对比组，并可选在并排画布左下角显示缩放比例。
- **文件变化保护**：内容修改会重新加载；文件新增、删除或重命名会冻结任务，必须显式刷新配对，避免比较过程中静默改变顺序。
- **审校标注模式**：从图源文件夹读取有效LabelPlus标注，在画布显示标注序号，并通过审校面板查看评审文本。
- **大图与快速翻页优化**：复用并排对比的左右 `ImageCanvas` 实例，避免连续切页重复创建组件；同时对缩略图、ImageBitmap、OpenCV 矩阵、Blob URL、Canvas 后备存储和邻近对比组预加载进行生命周期管理。PSD 使用独立解码 Worker、按显示用途分级解码，并只预加载前后各一组。
...
在首页左右两侧添加图片或文件夹。每一侧可以添加多个来源，并可在同侧拖拽调整文件夹顺序。选择文件夹时，如果目录连续嵌套且每层只有一个子文件夹，应用会直接添加最里面一层；判断时会忽略 `.DS_Store` 等隐藏文件。如果目录同时包含图片或其他可见内容，则保留当前选择的目录。文件夹只读取直属子文件中的支持图片，不递归扫描子目录；同一路径按绝对路径去重。

### 2. 确认配对结果

应用会先显示配对预览。默认按文件名去掉扩展名后进行精确匹配，并忽略大小写；形如 `06-07`、`06+07` 的纯数字组合文件名也会尝试匹配对应单页图片，无法精确匹配的图片按两侧排序结果依次配对。若需忽略文件名按顺序配对，可在配对预览旁打开“按顺序匹配”弹窗针对具体组别进行设置。确认顺序和缺图状态后进入工作区。

### 3. 进行图片对比

工作区提供并排、单页和分割三种模式。上下方向键切换对比组，按住左右方向键临时查看另一侧；常用色彩与滤镜参数会跨对比组及返回首页保留，可通过“重置所有”恢复默认值。

### 4. 进行审校与刷新

如果图片目录中存在符合格式的LabelPlus标注文件，可以打开审校模式，在画布和侧栏中查看标注。图片内容修改会重新加载；文件新增、删除或重命名会使任务进入待刷新状态，需要显式刷新配对后继续。
