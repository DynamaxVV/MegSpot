...
首页提供两栏式的成对图片对比流程：左侧为基准图，右侧为对比图。两侧均可拖入图片或文件夹，也可分别使用“选择图片”和“选择文件夹”。文件夹只读取直属图片；同一路径会自动去重。

- 先按忽略扩展名、忽略大小写的文件名精确匹配；其余图片再按各侧排序顺序依次配对，数量不一致时保留未匹配项。
- 两侧可独立按名称、修改时间或大小排序，并可选择升序或降序；最终配对整体顺序以左侧为主。
- 支持“并排”“单页”“分割”三种查看模式。单页模式默认满画布显示对比图；按住左右方向键可临时显示另一侧画面，松开即恢复。上下方向键用于切换上一组和下一组。
- 对比工作区支持空格键将当前组恢复到初始位置；双击单张图片可选中该图，使缩放、旋转等操作只作用于当前图片。
- 支持“原图模式/高清模式”，按设备像素比渲染画布；普通模式保留自适应显示。
- 图片内容修改会自动重新加载当前画面；文件新增、删除或重命名会冻结当前配对并提示手动“刷新配对”，避免比较次序发生变化。
- 每次重启均从空白首页开始，不恢复上一次配对任务；左右两侧最近一次通过文件夹选择器打开的目录会保留为快捷入口。

### 2.2.12-vv1.0.1 更新

- 修复首页多个文件夹来源的排序与配对顺序问题。
- 修复分割模式嵌入对比工作区时的画布尺寸和界面显示问题。
- 图片对比页新增空格键回正、双击选中单图缩放，以及原图/高清显示模式。
...
本仓库基于 GitHub 上游 MegSpot 2.2.12，当前修改版本为 2.2.12-vv1.0.1。除成对图片对比流程外，还包含多文件夹排序、分割模式显示、空格回正、双击单图缩放和原图/高清显示模式等更新。

本修改版本的标识为 `MegSpot-2.2.12-vv1.0.1`。本标识用于区分本次修改及其打包产物，不改变原项目 Apache License, Version 2.0 的许可条款；原项目版权和第三方依赖许可仍以仓库中的 `LICENSE`、`COPYRIGHT` 及相关声明为准。

Windows x64 打包结果：`MegSpot-2.2.12-vv1.0.0-win-x64.exe`（现有文件名）；后续 2.2.12-vv1.0.1 构建应使用对应版本号命名。
