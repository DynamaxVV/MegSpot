...
- **更可控的配对规则**：文件名匹配忽略扩展名和大小写；剩余图片按两侧排序顺序配对；文件夹只读取直属图片并按绝对路径去重。主页添加文件夹时，连续单子文件夹嵌套会定位到最里面一层。
...
应用会先显示配对预览。文件名去掉扩展名后进行精确匹配，并忽略大小写；形如 `06-07`、`06+07` 的纯数字组合文件名也会尝试匹配对应单页图片，无法精确匹配的图片按两侧排序结果依次配对。确认顺序和缺图状态后进入工作区。
