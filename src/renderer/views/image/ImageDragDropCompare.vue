    ...
    <div v-if="!embedded" class="buttons" flex="main:justify cross:center">
      <div class="changeButton">
        ...
        <el-button
          size="mini"
          @click="changeStatus"
          :title="$t(status ? 'imageDragDropCompare.hideLine' : 'imageDragDropCompare.displayLine')"
        >
          <svg-icon id="icon" :icon-class="status ? 'eye-open' : 'eye'" style="font-size: 12px"></svg-icon>
        </el-button>
        <span class="tip">
...
export default {
  ...
  methods: {
    ...
    //鼠标按下事件
    handleMouseDown(evt) {
      this.mouseLocation = this.windowToCanvas(this.canvas, evt.clientX, evt.clientY)
    ...
    handleMouseMove: throttle(30, function (event) {
      if (this.isMouseDown) {
        let mouseLocation = this.windowToCanvas(this.canvas, event.clientX, event.clientY)
    ...
    handleLineDown(evt) {
      ...
      this.content.onmousemove = (evt) => {
        ...
        if (this.flag) {
          newLinePosition = this.getUpdateLinePosition(oldLinePosition + changeY)
          this.topDis = newLinePosition + this.hoverHight / 2
          this.hoverLine.style.top = newLinePosition + 'px'
        } else {
          newLinePosition = this.getUpdateLinePosition(oldLinePosition + changeX)
    ...
    handleScroll(evt) {
      if (this.isInImage({ x: evt.clientX, y: evt.clientY })) {
        let newMouseLocation = this.windowToCanvas(this.canvas, evt.clientX, evt.clientY)
    ...
    isInImage(point) {
      ...
      let right = this.imagePosition.x + this.canvasPosition.left + this.initWidth * this.imgScale
      let top = this.imagePosition.y + this.canvasPosition.top
      let bottom = this.imagePosition.y + this.canvasPosition.top + this.initHeight * this.imgScale
    ...
    //切换对比方向
    changeDirection() {
      this.hoverLine.removeEventListener('mousedown', this.handleLineDown, false)
      this.imgScale = 1
      this.flag = !this.flag
      if (this.flag) {
