  ...
  <div id="image-edit" flex="dir:top">
    <div id="toolbar" flex="main:justify cross:center">
      <div id="left-bar" flex="main:center cross:center">
        <el-badge class="tool-item">
          <Gallery :selectedList="imageList" @update="setImages" @remove="removeImages">
        ...
        <el-select v-model="interVal">
          <el-option v-for="value in InterpolationFlags" :key="value" :label="value" :value="value"></el-option>
        </el-select>
        ...
        >
          <el-tooltip effect="light" placement="bottom">
            <div slot="content" id="rgba-region" flex="main:center cross:center">
              ...
              <el-slider id="slider" v-model="radius" show-input :min="0" :max="200"></el-slider>
            </div>
            <div
              id="rgba-block"
              :style="{
    ...
    <div id="main-content" flex-box="1">
      <div
        ...
        "
      >
        <canvas ref="hist" v-show="histVisible" id="hist" @click="histVisible = false"></canvas>
        <i v-show="!histVisible" id="hist-icon" @click="histVisible = true"><svg-icon icon-class="chart"></svg-icon></i>
      </div>
      <div id="outline-container">
        <img v-show="outlineVisible" id="outline-img" @click="outlineVisible = false" :src="imgSrc" />
        <div id="outline-roi" v-show="outlineVisible" @click="outlineVisible = false"></div>
        <i v-show="!outlineVisible" id="outline-icon" class="el-icon-picture" @click="outlineVisible = true"></i>
      </div>
      <OperationContainer
        id="canvas-container"
...
export default {
  ...
  methods: {
    ...
    redraw() {
      const roiMat = this.sourceMat.roi(new cv.Rect(this.roiPos.rx, this.roiPos.ry, this.roiPos.rw, this.roiPos.rh))
      this.currentMat && this.currentMat.delete()
      this.currentMat = this.changeSize(roiMat, this.canvasPos.cw, this.canvasPos.ch)
    ...
    getFormatHistArea() {
      return {
        x: this.histPos.hw > 0 ? this.histPos.hx : this.histPos.hx + this.histPos.hw,
        y: this.histPos.hh > 0 ? this.histPos.hy : this.histPos.hy + this.histPos.hh,
    ...
    getRGBA(mousePos) {
      ...
      if (this.radius > 0) {
        ...
        const x_max = x + offset <= this.currentMat.cols ? x + offset : this.currentMat.cols
        const y_min = y - offset >= 0 ? y - offset : 0
        const y_max = y + offset <= this.currentMat.rows ? y + offset : this.currentMat.rows
        const matData = this.currentMat.colRange(x_min, x_max).rowRange(y_min, y_max).data
    ...
    handleZoom: throttle(50, function (rate, mousePos) {
      const { x: roix, y: roiy } = this.transformPosToROI(mousePos, this.roiPos)
    ...
    calcHist(histPos) {
      return Promise.resolve().then(() => {
        ...
        if (histPos) {
          const roiMat = this.sourceMat.roi(new cv.Rect(histPos.x, histPos.y, histPos.w, histPos.h))
